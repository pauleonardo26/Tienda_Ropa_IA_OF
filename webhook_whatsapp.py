...
from pedido_carrito import (
    MAX_PRODUCTOS_PEDIDO,
...
# =========================================================
# 06 - TARJETA 1 - BIENVENIDA / NIÑO O NIÑA
# =========================================================
...
# =========================================================
# 07 - TARJETA 2 - ROPA PARA NIÑA
# =========================================================

def enviar_tarjeta_nina(numero_destino):

    print(
        "Enviando Tarjeta 2 - Ropa para niña a:",
        numero_destino
    )
...
def enviar_tarjeta_nino(numero_destino):
    ...
    print(
        "Enviando menú Ropa para niño a:",
        numero_destino
    )
...
# =========================================================
# 09 - FUNCIÓN BASE - TARJETA DE PRODUCTO NIÑA
# =========================================================

def enviar_tarjeta_producto_nina(
    numero_destino,
    nombre_producto,
    imagen_url,
    opciones
...
# =========================================================
# 11 - CONSULTAR PRODUCTO REAL EN POSTGRESQL
# =========================================================
...
# =========================================================
# 12 - DESCARGAR FOTO DESDE CLOUDFLARE R2
# =========================================================
    ...
    datos = {
        ...
        **destino,
        ...
        "image": {
            "link": (
                "https://pub-9aa04db1bd594751a5b8fb2654da15fd.r2.dev/"
                "file_00000000b484820e9b52ed1bc4943655.png"
            )
        }
    }
...
def validar_pedido_postgresql(pedido_interpretado):
    ...
    productos_pedido = pedido_interpretado.get(
        "productos",
        []
    )
    ...
    try:
        ...
        for item in productos_pedido:
            ...
            # ---------------------------------------------------------
            # 19.1 - DEFINIR POLO M/C POR TALLA
            # ---------------------------------------------------------
            ...
            if nombre_real == "POLO_MANGA_CORTA":
                ...
                if conjunto_tallas.issubset(
                    ...
                    errores.append(
                        "⚠️ En polo manga corta mezclaste "
                        "tallas pequeñas y grandes. "
                        "Escríbelas por separado."
                    )

                    continue

            # ---------------------------------------------------------
            # 19.2 - DEFINIR POLO M/L POR TALLA
            # ---------------------------------------------------------
            ...
            if nombre_real == "POLO_MANGA_LARGA":
                ...
                if conjunto_tallas.issubset(
                    ...
                    errores.append(
                        "⚠️ En polo manga larga mezclaste "
                        "tallas pequeñas y grandes. "
                        "Escríbelas por separado."
                    )
            ...
            # ---------------------------------------------------------
            # 19.4 - DISTRIBUIR CANTIDAD ENTRE TALLAS
            # ---------------------------------------------------------
            ...
            if len(tallas) == 1:
                ...
                errores.append(
                    f"⚠️ {nombre_real}: pediste "
                    f"{cantidad} unidades pero indicaste "
                    f"{len(tallas)} tallas "
                    f"({', '.join(tallas)}). "
                    "Indícame cuántas unidades quieres "
                    "de cada talla."
                )
            ...
            for talla, color in zip(
                ...
                pedidos_unidad[clave] = (
                    pedidos_unidad.get(clave, 0) + 1
                )

            # ---------------------------------------------------------
            # 19.6 - VALIDAR TALLA Y COLOR EN POSTGRESQL
            # ---------------------------------------------------------

            for (
                talla,
                color_solicitado
            ), cantidad_unidades in pedidos_unidad.items():
                ...
                if not variantes:
                    ...
                    errores.append(
                        ...
                        f"{', '.join(tallas_disponibles)}"
                    )

                    continue

                # -----------------------------------------------------
                # 19.8 - PRODUCTO CON COLOR SURTIDO
                # -----------------------------------------------------
                ...
                # -----------------------------------------------------
                # 19.9 - PRODUCTO CON COLORES REALES
                # -----------------------------------------------------

                colores_db = {
                    str(variante[2]).strip().lower():
                    variante
                    for variante in variantes
                }
    ...
    try:
        ...
        if contactos:
            ...
            if not numero_cliente:
                ...
                numero_cliente = contacto.get(
                    "wa_id"
                )

            if not bsuid_cliente:

                bsuid_cliente = contacto.get(
                    "user_id"
                )
        ...
        if tipo_mensaje == "interactive":
            ...
            tipo_interactivo = interactive.get(
                "type"
            )
            ...
            if tipo_interactivo == "button_reply":
                ...
                if boton_id == "continuar_pedido":

                    cotizacion = (
                        COTIZACIONES_CLIENTES.get(
                            numero_cliente
                        )
                    )
                    ...
                    if cantidad_productos >= MAX_PRODUCTOS_PEDIDO:
                        ...
                        enviar_mensaje(
                            numero_cliente,
                            (
                                f"ℹ️ Ya alcanzaste el máximo "
                                f"de {MAX_PRODUCTOS_PEDIDO} "
                    ...
                    enviar_mensaje(
                        ...
                        (
                            ...
                            "Elige una prenda del catálogo "
                            "y luego presiona *HACER PEDIDO*."
                        )
                    )
                ...
                # =================================================
                # 20.5.9 - FINALIZAR PEDIDO
                # =================================================

                if boton_id == "finalizar_pedido":
                ...
                if boton_id == "hacer_pedido":
                    ...
                    if cotizacion_existente:
                        ...
                        enviar_mensaje(
                            ...
                            (
                                "🛍️ *HACER MI PEDIDO*\n\n"
                                "Escríbeme todo lo que deseas "
                                "pedir en un solo mensaje.\n\n"
                                "Ejemplo:\n"
                                "3 leggings negros talla 8 "
                                "y 2 polos manga corta talla 4."
                            )
                        )
            ...
            if tipo_interactivo == "list_reply":
                ...
                respuestas_nino = {
                    "nino_polos": (
                        "👕 Elegiste *POLOS DE NIÑO*.\n\n"
                        "Próxima tarjeta a conectar."
                    ),
                    "nino_joggers": (
                        "👖 Elegiste *JOGGERS DE NIÑO*.\n\n"
                        "Próxima tarjeta a conectar."
                    ),
                    "nino_conjunto": (
                        "🧢 Elegiste *CONJUNTO DE NIÑO*.\n\n"
                        "Próxima tarjeta a conectar."
                    )
                }
        ...
        # =========================================================
        # 20.9 - ESTADO ACTUAL DEL CLIENTE
        # =========================================================
        ...
        # =========================================================
        # 20.10 - SALIR DEL PEDIDO SI PIDE CATALOGO
        # =========================================================
        ...
        if estado_actual in [
            ...
            total_nuevo = validacion.get(
                "total",
                0
            )

            # -----------------------------------------------------
            # 20.11.4 - ERRORES DE VALIDACION
            # -----------------------------------------------------
            ...
            # -----------------------------------------------------
            # 20.11.5 - RECUPERAR COTIZACION EXISTENTE
            # -----------------------------------------------------
            ...
            # -----------------------------------------------------
            # 20.11.6 - LIMITE DE 5 PRODUCTOS
            # -----------------------------------------------------

            if not resultado_carrito["ok"]:

                enviar_mensaje(
                    numero_cliente,
                    resultado_carrito["error"]
                )
            ...
            # -----------------------------------------------------
            # 20.11.7 - GUARDAR COTIZACION ACUMULADA
            # -----------------------------------------------------
